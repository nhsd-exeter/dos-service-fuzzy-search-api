...
@SpringBootTest(classes = {FuzzySearchControllerAdviceTest.Config.class, FuzzySearchControllerAdviceTest.class, FuzzyServiceSearchController.class, FuzzySearchControllerAdvice.class})
public class FuzzySearchControllerAdviceTest {
  ...
  @BeforeEach
  public void setup() {
    mockMvc = MockMvcBuilders.standaloneSetup(controller).setControllerAdvice(new FuzzySearchControllerAdvice()).build();
  }

  @Test
  public void testHappyPath() throws Exception {
    mockMvc.perform(get("/dosapi/dosservices/v0.0.1/services/byfuzzysearch")
  ...
  public void testInvalidParameterException() throws Exception {
    when(fuzzyServiceSearchService.retrieveServicesByGeoLocation(any(), any(), any(), any(), any()))
      .thenThrow(new InvalidParameterException("None of the search criteria given meets the minimum required search criteria length."));
    ...
    mockMvc.perform(get("/dosapi/dosservices/v0.0.1/services/byfuzzysearch")
      ...
      .andExpect(content().json("{\"validationCode\":\"PMA-VAL003\",\"message\":\"None of the search criteria given meets the minimum required search criteria length.\"}"));
  }
  ...
  public void testNormalException() throws Exception {
    ...
    ResponseEntity<ErrorMessage> errorResponse = advice.handleException(new Exception("An internal test error has occurred"));
