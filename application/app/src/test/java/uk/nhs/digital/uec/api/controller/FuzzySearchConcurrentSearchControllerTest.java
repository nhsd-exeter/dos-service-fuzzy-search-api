...
class FuzzySearchConcurrentSearchControllerTest {
  ...
  @Mock private ConcurrentFuzzySearchService concurrentFuzzySearchService;

  @Mock private ApiUtilsServiceInterface utils;

  @Mock private ApiRequestParams requestParams;

  @InjectMocks private FuzzySearchConcurrentSearchController fuzzySearchController;
  ...
  @Test
  void getServicesByFuzzySearch()
      throws InterruptedException, ExecutionException, InvalidParameterException,
          NotFoundException {
    List<String> searchCriteria = Arrays.asList("term1", "term2");
    ...
    doNothing()
        .when(utils)
        .configureApiRequestParams(
            eq(fuzzLevel),
            any(),
            any(),
            eq(maxNumServicesToReturn),
            eq(namePriority),
            eq(addressPriority),
            eq(postcodePriority),
            eq(publicNamePriority));
    ...
    CompletableFuture<List<DosService>> dosServicesFuture =
        CompletableFuture.completedFuture(mockDosServices);

    when(concurrentFuzzySearchService.fuzzySearch(
            eq(searchLatitude),
            eq(searchLongitude),
            eq(distanceRange),
            eq(searchCriteria),
  ...
  @Test
  void getServicesByFuzzySearch_testMaximumNumberOfServices()
      throws InterruptedException, ExecutionException, InvalidParameterException,
          NotFoundException {
    List<String> searchCriteria = Arrays.asList("term1", "term2");
    String searchPostcode = "WV10 6BZ";
  ...
  @Test
  void getServicesByFuzzySearch_testConcurrentRequests()
      throws InterruptedException, ExecutionException, InvalidParameterException,
          NotFoundException {
    List<String> searchCriteria = Arrays.asList("term1", "term2");
    String searchPostcode = "WV10 6BZ";
    ...
    CompletableFuture<ResponseEntity<ApiResponse>>[] futures =
        new CompletableFuture[numberOfThreads];
