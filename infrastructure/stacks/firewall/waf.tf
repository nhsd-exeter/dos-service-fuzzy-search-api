#Uncomment incase state is loss on AWS
...
resource "aws_wafv2_web_acl_logging_configuration" "waf_acl_lc" {
  log_destination_configs = [data.aws_cloudwatch_log_group.waf_logs.arn]
  resource_arn            = data.aws_wafv2_web_acl.waf_acl.arn
  lifecycle {
    create_before_destroy = true
  }
}
