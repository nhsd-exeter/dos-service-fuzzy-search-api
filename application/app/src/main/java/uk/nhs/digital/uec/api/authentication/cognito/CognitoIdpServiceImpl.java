...
public class CognitoIdpServiceImpl implements CognitoIdpService {
  ...
  private AuthToken getMockAuthenticationToken(Credential credential){
    ...
    if(userName.equalsIgnoreCase("service-finder-admin@nhs.net") && pass_word.equalsIgnoreCase("mock-auth-pass")){
      log.info("Returning Mock Token");
      return new AuthToken(MockAuthenticationConstants.MOCK_ACCESS_TOKEN,MockAuthenticationConstants.MOCK_ACCESS_TOKEN);
    }else {
